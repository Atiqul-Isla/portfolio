            ...
            <div className='grid place-content-center mt-12'>
                <div className='grid gap-6 max-w-5xl'>
                 {WorkData.map((item) => {
                    return(
                        <>
                        <motion.div layout className='w-[75vh] flex flex-col '>
                            <motion.div transition={{layout: 'spring'}} key={item.id} className=' bg-gray-900 rounded-lg shadow-lg shadow-black'>
                                <motion.div className='flex justify-between ml-2'>
                                    <motion.div >
                                        <motion.p className='pl-2 pt-2 font-bold uppercase'>{item.jobName}</motion.p>
                                        <motion.span  className=' pb-2 text-purple-400 pl-2 font-bold opacity-80'>{item.companyName}</motion.span>
                                    </motion.div>
                            ...
                            {!open[item.id] && 
                            <motion.div layout className='bg-gray-300 rounded-lg pl-2 mt-4' initial={{opacity: 0, y:-30}} animate={{opacity: 1, y:0}} exit={{opacity: 0}} transition={{duration: 0.5, delay: 0.3}}>
